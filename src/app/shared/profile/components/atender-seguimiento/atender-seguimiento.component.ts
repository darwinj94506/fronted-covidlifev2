...
export class AtenderSeguimientoComponent extends Formulario implements OnInit {
  ...
  initForm(){
    this.seguimientoForm = this.fb.group({
      ...
      ritmo_cardiaco: [ { value: this.getRitmoSaturacion(this.seguimiento.ritmo_cardiaco), disabled: this.isDisabled() },
         [Validators.pattern("^[0-9]*$"), Validators.min(40), Validators.max(200)] ],
      saturacion_oxigeno: [{ value: this.getRitmoSaturacion(this.seguimiento.saturacion_oxigeno), disabled: this.isDisabled() }, 
        [Validators.pattern("^[0-9]*$"), Validators.min(2), Validators.max(100)] ],
